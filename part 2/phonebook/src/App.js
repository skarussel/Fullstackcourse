  ...
  const addName = (event) => {
    ...
    if (duplicate) {
      ...
      if (
        window.confirm(
          newName +
            " is already a member of the Phonebook. Replace old number with the new one?"
        )
      ) {
        const nameObject = {
          name: newName,
...
const Notification = ({ update_msg, error_msg }) => {
  if (update_msg === null && error_msg === null) {
    ...
    return <div className="notification">{update_msg}</div>;
  } else {
...
const Filter = ({ filter, filterFunc }) => {
  return (
    <div>
      filter shown with a <input value={filter} onChange={filterFunc} />
    </div>
  );
};
